...
public class FrmPapeleta extends JInternalFrame implements ActionListener, MouseListener {
	...
	private JComboBox cboPolicia;
	private JComboBox cboInfractor;
	...
	private ArrayList<String> codPol = new ArrayList<String>();
	private ArrayList<String> codInf = new ArrayList<String>();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrmPapeleta frame = new FrmPapeleta();
	...
	private void Limpiar(){
		txtCodigo.setText("");
		cboPolicia.setSelectedIndex(0);
		cboInfractor.setSelectedIndex(0);
		dcFecha.setCalendar(new GregorianCalendar());
		txtMonto.setText("");
	...
	private void Habilitar(boolean estado){
		...
		cboPolicia.setEnabled(estado);
		cboInfractor.setEnabled(estado);
		dcFecha.setEnabled(estado);
		txtMonto.setEnabled(estado);
		txtDescripcion.setEnabled(estado);
	}
	
	private void LLenaPol(){
		ArrayList<Policia> lista = new ArrayList<Policia>();
		lista = mPol.listarPol();
		codPol.clear();
		cboPolicia.removeAllItems();
		for (Policia item : lista){
			cboPolicia.addItem(item.getApe());
			codPol.add(item.getCodigo());
		}
	}
	
	private void LLenaInf(){
		ArrayList<Infractor> lista = new ArrayList<Infractor>();
		lista = mInf.listarInf();
		codInf.clear();
		cboInfractor.removeAllItems();
		for (Infractor item : lista){
			cboInfractor.addItem(item.getApe());
			codInf.add(item.getCodigo());
		}
	}
	...
	private void InsertarPap(){
		...
		p.setCodPol(codPol.get(cboPolicia.getSelectedIndex()));
		p.setCodInf(codInf.get(cboInfractor.getSelectedIndex()));
	...
	private void initComponents() {
		...
		cboPolicia = new JComboBox();
		cboPolicia.setBounds(102, 67, 160, 20);
		panel.add(cboPolicia);
		
		cboInfractor = new JComboBox();
		cboInfractor.setBounds(102, 106, 160, 20);
		panel.add(cboInfractor);
	...
	protected void mouseReleasedTblPapeletas(MouseEvent arg0) {
		if(!tblPapeletas.getSelectionModel().isSelectionEmpty()){
			...
			cboPolicia.setSelectedItem(tblPapeletas.getValueAt(fila, 1).toString());
			cboInfractor.setSelectedItem(tblPapeletas.getValueAt(fila, 2).toString());
			try {
